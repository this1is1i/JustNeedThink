    ...
    <div className="flex h-full flex-col" style={{ backgroundColor: 'var(--color-bg-secondary)' }}>
      ...
      <div className="border-b" style={{ borderColor: 'var(--color-border)' }}>
        ...
        {activeAgents.length === 0 ? (
          <div className="px-3 pb-3 text-sm" style={{ color: 'var(--color-text-muted)' }}>
            Idle
          </div>
        ) : (
          activeAgents.map((a) => (
            <div key={a.id} className="border-t px-3 py-2 text-sm" style={{ borderColor: 'var(--color-border)' }}>
              <div className="flex items-center gap-2">
                <span className="text-xs" style={{ color: 'var(--color-accent)' }}>{phaseIcon(a.phase)}</span>
                <span style={{ color: 'var(--color-text)' }}>{a.name}</span>
              </div>
              <div className="text-xs" style={{ color: 'var(--color-text-muted)', marginLeft: '20px' }}>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {definitions.map((d) => (
          <div key={d.id} className="border-t px-3 py-3 text-sm" style={{ borderColor: 'var(--color-border)' }}>
            <div className="flex items-center gap-2">
            ...
            <div className="mt-1 text-xs leading-5" style={{ color: 'var(--color-text-muted)' }}>{d.description}</div>
            {d.tools.length > 0 && (
      ...
      <div className="border-t" style={{ borderColor: 'var(--color-border)' }}>
        ...
        {teams.map((t) => (
          <div key={t.id} className="border-t px-3 py-2 text-sm" style={{ borderColor: 'var(--color-border)' }}>
